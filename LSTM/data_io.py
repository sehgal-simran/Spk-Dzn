...
def make_k_fold_dataset(rec_ids, rec_batches, base_path, k=5):
    '''
    rec_ids: all recording ids from segments, iaaa, iaab
    '''
    ...
    for i, te in enumerate(tqdm(splits)):
        ...
        # get the test data's corresponding xvector paths, speakers, segment lines
        utts, paths, spkrs, seglines = get_subset_files(test_ids, test_batches)
        # make those information into files
...
def group_recs(utt2spk, segments, xvecscp):
    '''
    for each unique recording id (iaaa), group its information (xvector path, segment, etc.)
    '''
